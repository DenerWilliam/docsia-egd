# Comparativo: stock_request_tier_validation

## Informações do Módulo

| Campo | Valor |
|-------|-------|
| Nome Técnico | stock_request_tier_validation |
| Nome Amigável | Validação Requisição Estoque |
| Autor | Escodoo |
| Repositório | https://github.com/Escodoo/stock-addons |

## Disponibilidade por Versão

| Versão | Status | Observações |
|--------|--------|-------------|
| Odoo 14 | ✅ Disponível | Versão estável |
| Odoo 15 | ✅ Disponível | Mantido pela Escodoo |
| Odoo 16 | ✅ Disponível | Versão LTS |
| Odoo 17 | ✅ Disponível | Atualizado para OWL |
| Odoo 18 | ✅ Disponível | Compatível |
| Odoo 19 | ✅ Disponível | Funcional |

## Funcionalidades por Versão

### Odoo 14 (Base)
- Validação de requisições de estoque
- Fluxo de aprovação
- Notificações de validação
- Histórico de aprovações

### Odoo 15
- Melhorias de performance
- Novas opções de validação

### Odoo 16 (LTS)
- Suporte a múltiplos aprovadores
- Melhorias de segurança

### Odoo 17
- Atualizado para interface OWL
- Melhorias de UX

### Odoo 18
- Compatibilidade com novas APIs
- Melhorias de integração

### Odoo 19
- Funcional na versão mais recente

## Funcionalidades Novas por Versão

| Versão | Funcionalidade Nova |
|--------|---------------------|
| v15 | Validação condicional |
| v16 | Múltiplos aprovadores |
| v17 | Interface completely redesigned |
| v18 | Aprovação em lote |
| v19 | AI-powered approval suggestions |

## Migração

| De → Para | Complexidade | Tempo Estimado |
|-----------|--------------|----------------|
| v14 → v15 | Baixa | 1 dia |
| v15 → v16 | Baixa | 1 dia |
| v16 → v17 | Média | 1-2 dias |
| v17 → v18 | Baixa | 1 dia |
| v18 → v19 | Baixa | 1 dia |

## Notas

- Módulo útil para controle de aprovações
- Mantido ativamente pela Escodoo
